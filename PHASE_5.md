...
Motivation: The current architecture (Phases 1-3) established a solid foundation with LINQ.js integration and a three-layer model. However, several areas still use imperative, configuration-heavy patterns instead of the composable, functional style that LINQ exemplifies. Phase 5 addresses this by:
...
High Priority (Core Composability): 5a, 5b, 5c - Foundation for all improvements
Medium Priority (Developer Experience): 5d, 5f - Make the API ergonomic
Lower Priority (Advanced Features): 5e, 5g - Nice-to-have enhancements
...
Create QueryBuilder class with fluent API
Implement immutable builder pattern (each method returns new instance)
Create Engine interface with query() factory method
Update runQuery to accept QuerySpec internally
Add helper methods: forMetric(name), byAttributes(...attrs)
Benefits:
...
Define new MetricContext and MetricEval types
Create constructor functions for each metric type
Implement evaluateMeasure and evaluateDependencies helpers
Update evaluateMetric to work with new unified interface
Migrate existing metrics to new format
Add higher-order helpers (makeYtdMetric, makeYoYMetric, etc.)
Remove old tagged union types
Benefits:
...
Update ContextTransform type to work with MetricContext
Implement composeTransforms utility
Refactor existing transforms to be pure functions
Create parameterized transform builders
Add library of common transforms (rolling windows, shifts, etc.)
Update documentation with composition examples
Benefits:
...
Define filter AST types
Create builder API (f.* functions)
Implement compileFilter function
Update applyContextToFact to accept Filter AST
Maintain backward compatibility with plain object filters
Add filter optimization rules (e.g., merge adjacent conditions)
Update query builder to accept filters
Benefits:
...
Define SemanticModel type
Define Engine interface
Implement createEngine factory function
Implement Engine.extend() for composition
Update QueryBuilder to receive engine in constructor
Migrate all demos to use engine pattern
...
Create attr builder namespace
Create measure builder namespace
Create metric builder namespace (extends 5b)
Add validation helpers (check column exists in table)
Migrate demo definitions to use builders
...
Create new test file: semanticEngine.composition.test.js
Add transform composition tests
Add query builder composition tests
Add metric combinator tests
Add filter composition tests
Add end-to-end integration tests
Update TEST_PLAN.md with composition test coverage
Benefits:
...
5e. Model/Engine Separation (6-8h) - Foundation for everything else
5a. Query Builder Pattern (6-8h) - Core UX improvement
5c. Composable Transforms (4-6h) - Quick win, builds on existing
5b. Metric Combinators (10-12h) - Largest refactor, most impactful
5f. DSL Helpers (4-6h) - Polish after core is done
5d. Filter Expression Language (8-10h) - Advanced feature
5g. Composition Testing (6-8h) - Continuous throughout
Breaking Changes:
